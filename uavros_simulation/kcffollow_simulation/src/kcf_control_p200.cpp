...
Eigen::Vector3d PX4Landing::KcfPidProcess(Eigen::Vector4d &currentAngle,float expectAngle_x,float expectAngle_y) 
{
	...
	/* 前进速度v的控制 位置式PID*/
	PIDv.difference = expectAngle_y-currentAngle[1]; //v朝机头方向
	//PIDv.difference = expectAngle_y-currentAngle[0]; //v朝机身右方x方向，还要修改为kcf_angle[0]=-Detection_raw.angle_x
	PIDv.differential = PIDv.difference - PIDv.tempDiffer;
	PIDv.intergral += PIDv.difference;
	if (PIDv.intergral>10)
		PIDv.intergral = 10;
	if (PIDv.intergral<-10)
		PIDv.intergral = -10;
	v_out = kcfPID_v.p*PIDv.difference + kcfPID_v.d*PIDv.differential + kcfPID_v.i*PIDv.intergral;
	PIDv.tempDiffer = PIDv.difference;

	/* 转动速度w的控制 位置式PID*/
	PIDw.difference = expectAngle_x-currentAngle[0]; //v朝机头方向
	//PIDw.difference = expectAngle_x-currentAngle[1]; //v朝机身右方x方向
	PIDw.differential = PIDw.difference - PIDw.tempDiffer;
	PIDw.intergral += PIDw.difference;
	if (PIDw.intergral>10)
		PIDw.intergral = 10;
	if (PIDw.intergral<-10)
		PIDw.intergral = -10;
...
void PX4Landing::Initialize()
{
  ...
  s_PidItemYaw.intergral = 0;//for ar tag landing
	PIDv.tempDiffer = 0; //for kcf tracking
	PIDv.intergral = 0;//for kcf tracking
	PIDw.tempDiffer = 0;//for kcf tracking
	PIDw.intergral = 0;//for kcf tracking
