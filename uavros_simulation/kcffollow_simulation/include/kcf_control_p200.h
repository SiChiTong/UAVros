...
#include "uavros_msgs/KcfDetectioninfo.h" //spx
...
class PX4Landing {
 ...
 private:
  ...
  void KcfAngleCallback(const uavros_msgs::KcfDetectioninfo::ConstPtr &msg);
  ...
  Eigen::Vector3d KcfPidProcess(Eigen::Vector4d &currentAngle,float expectAngle_x,float expectAngle_y);
  ...
  uavros_msgs::KcfDetectioninfo Detection_raw;// kcf跟踪原始数据
  bool kcf_state; // kcf是否在跟踪目标标志位
  bool detect_state;//是否检测到降落板二维码标志位
  ...
  S_PID s_PidXY,s_PidZ,s_PidYaw;//二维码降落用
  S_PID kcfPID_v,kcfPID_w; //kcf 跟踪用
  S_PID_ITEM s_PidItemX; //二维码降落用
  S_PID_ITEM s_PidItemY; //二维码降落用
  S_PID_ITEM s_PidItemZ; //二维码降落用
  S_PID_ITEM s_PidItemYaw; //二维码降落用
  S_PID_ITEM PIDv; //kcf 跟踪用
  S_PID_ITEM PIDw; //kcf 跟踪用
  ...
  enum
 {
  ...
  ros::Subscriber kcf_angle_sub;
  ros::Subscriber position_sub_;
  ros::Subscriber rel_alt_sub_;
  ros::Subscriber state_sub_;
  ros::ServiceClient set_mode_client_;
  ros::ServiceClient arming_client_; //spx
};
