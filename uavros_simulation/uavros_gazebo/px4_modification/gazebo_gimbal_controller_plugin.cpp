...
void GimbalControllerPlugin::SendMavlinkMessage(const mavlink_message_t &msg)
{
  ...
  // udp_gimbal_port_remote added by spx to distinguish multiple gimbals
  // this bug has been fixed in px4 v1.14 ?
  int udp_gimbal_port_remote;
  if (this->sdf->HasElement("control_gimbal_channels")) {
    udp_gimbal_port_remote = this->sdf->Get<int>("udp_gimbal_port_remote");
  }
  else{
    udp_gimbal_port_remote = 13030;
  }
  dest_addr.sin_port = htons(udp_gimbal_port_remote);
  // gzwarn << "[gimbal_controller_plugin] dest_addr.sin_port: " << udp_gimbal_port_remote << "\n";
