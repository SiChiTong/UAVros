...
Since Gazebo also publishes data faster than normal odom data, this script caps the update to 50hz.
Winter Guerra
'''
#just publish xyz position currently
import rospy
...
class OdometryNode:
    # Set publishers
    pub_odom = rospy.Publisher('modelpos_gazebo', Odometry, queue_size=1)
    ...
    def __init__(self):
        ...
        self.Modelname = rospy.get_param('~modelname','iris') #'~' means param in private namespace
        self.Linkname = rospy.get_param('~linkname','base_link') #'~' means param in private namespace
        #self.ID = rospy.get_param('modelID','None')
        #print('modelID odometry:', self.ID)
    ...
    def sub_robot_pose_update(self, msg):
        try:
            arrayIndex = msg.name.index(str(self.Modelname)+'::'+str(self.Linkname))
        except ValueError as e:
            # Wait for Gazebo to startup
            print('[gazebo_modelpos_publisher]: wrong model::link name from topic /gazebo/link_states!')
            pass
        else:
    ...
    def timer_callback(self, event):
        ...
        cmd.child_frame_id = str(self.Linkname)
...
if __name__ == '__main__':
    rospy.init_node("gazebo_modelpos_publisher")
    node = OdometryNode()
    rospy.spin()
