...
* Time: 2021.5.17
* Description: 实现px4 quadrotor 二维码精准着陆，作为一个service模块
***************************************************************************************************************************/
#include "landing_service.h"
using namespace std;
...
PX4Landing::PX4Landing(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private):
  ...
  cmd_sub = nh_.subscribe("land_command",10,&PX4Landing::cmdCallback,this,ros::TransportHints().tcpNoDelay()); 

  landLocation_sub_ = nh_.subscribe("/ugv3/odom", 10, &PX4Landing::landLocation_cb, this, ros::TransportHints().tcpNoDelay());

  cmd_pub_ = nh_.advertise<std_msgs::Int32>("/cmd", 10);
  // 【服务】修改系统模式
  set_mode_client_ = nh_.serviceClient<mavros_msgs::SetMode>("mavros/set_mode");

  // 【服务】解锁/上锁 spx  include文件中还要定义arming_client和arm_cmd！
  arming_client_ = nh_.serviceClient<mavros_msgs::CommandBool>("mavros/cmd/arming");
  precise_landing_service_ = nh_.advertiseService("precise_landing", &PX4Landing::landCallback, this);
}
	...
	switch(LandingState)
	{
		case WAITING:
			...
			*/
			...
			if(detect_target_state == true)
			{
				if(last_detect_target_state == false)
				{  //avoid publishing to cmd_pub_ repeatedly after finding the target
					std_msgs::Int32 cmd_pub_msg;
					//cmd_pub_msg.data = 0;
					cmd_pub_.publish(cmd_pub_msg);
					cout << "find the target" << endl;
				}
				last_detect_target_state = true;
			}
			else {last_detect_target_state = false;}
		...
		*/
		...
		case PREPARE:	//起飞到指定高度
			...
			{ //如果在准备中途中退出offboard切换到手动，则跳到WAITING 
				LandingState = WAITING;
				cout << "LandingState: WAITING" <<endl;
				command.data = 0;
			}
			last_px4_state_ = px4_state_;
			//cout << "PREPARE" <<endl;
			break;
		...
		case SEARCH:
			if(detect_big == true)
			{
				LandingState = APPROACH;
			  	cout << "LandingState: APPROACH" <<endl;
			}	
			else//搜寻目标
			{
				posxyz_target[0] = landLocation_pos_[0];  //时刻给降落点位置（uwb保证精度）;
				posxyz_target[1] = landLocation_pos_[1];  //时刻给降落点位置（uwb保证精度）;
		...
		case APPROACH:
			{
				...
				//the height may jump due to the high car surface
				...
				if((detect_state == true) && approach_count == Thres_count_approach)
				{
					...
					approach_count = 0;
					cout << "LandingState: LANDING" <<endl;
				}				
			}
			break;
/*
		...
		case LANDING:
			{				
				...
				if(detect_small == true && small_count > Thres_count_small && px4_pose_[2] < Thres_height_small)
				{
					...
					small_count = 0;
					cout << "LandingState: LANDING_SMALL" <<endl;  
				}
...
//			cout << "markers_yaw_: "  << markers_yaw_ << endl;
		...
		if(item.id == target_marker_id_)
		{
			detect_target_state = true;
		}
	}
...
{
	command = *msg;
	cout << "landing service node receive command: " << command.data << endl;
}

void PX4Landing::landLocation_cb(const nav_msgs::Odometry &msg)
{ //gazebo仿真中飞机原点是起飞点不同于map原点
  landLocation_pos_(0) = msg.pose.pose.position.x - init_x_;
  landLocation_pos_(1) = msg.pose.pose.position.y - init_y_;
  landLocation_pos_(2) = 0.7;
  /*
...
bool PX4Landing::landCallback(std_srvs::SetBool::Request &request, std_srvs::SetBool::Response &response) 
{
  response.success = true;
  if(LandingState == WAITING)
  {
	response.message = "Receive precise landing service call";
	LandingState = PREPARE;
	cout << "LandingState: PREPARE" <<endl;
  }
  else
  {
	response.message = "Receive precise landing service call, But already in landing process";
  }
  return true;
}
...
void PX4Landing::Initialize()
{
  ...
  nh_private_.param<int>("big_marker_id", big_marker_id_, 4);
  nh_private_.param<int>("small_marker_id", small_marker_id_, 0);
  nh_private_.param<int>("target_marker_id", target_marker_id_, 6);
  ...
  nh_private_.param<float>("init_x", init_x_, 0); //gazebo仿真中飞机原点是起飞点不同于map原点
  nh_private_.param<float>("init_y", init_y_, 0);
  ...
  detect_small = false;
  detect_target_state = false;
  last_detect_target_state = false;
  desire_vel_[0] = 0;
  desire_vel_[1] = 0;
  desire_vel_[2] = 0;
  desire_vel_[3] = 0;
  ...
  landLocation_pos_ << 1.0, 1.0, 0.7; //avoid flying to the origin
  ...
  command.data = 0;
	...
	cout << "target_marker_id = " << target_marker_id_ << endl;
	...
	cout << "init_x = " << init_x_ << endl;
	cout << "init_y = " << init_y_ << endl;
