...
PX4Landing::PX4Landing(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private):
  ...
  cmd_sub = nh_private_.subscribe("/command",10,&PX4Landing::cmdCallback,this,ros::TransportHints().tcpNoDelay()); 
  // 【服务】修改系统模式
...
//	cout << "s_PidItemX.differential: " << s_PidItemX.differential << endl;
  ...
  s_PidItemY.tempDiffer = s_PidItemY.difference;
	...
	if(s_PidItemZ.intergral >= 1)		
		s_PidItemZ.intergral = 1;
	else if(s_PidItemZ.intergral <= -1) 
		s_PidItemZ.intergral = -1;
  ...
  s_PidItemZ.tempDiffer = s_PidItemZ.difference;
	...
	if(s_PidOut[2] >= 1)		
		s_PidOut[2] = 1;
	else if(s_PidOut[2] <= -1) 
		s_PidOut[2] = -1;

	/*Yaw方向的pid控制*/
	s_PidItemYaw.difference =  expectYaw - currentYaw;
	s_PidItemYaw.intergral += s_PidItemYaw.difference;
	if(s_PidItemYaw.intergral >= 1)		
		s_PidItemYaw.intergral = 1;
	else if(s_PidItemYaw.intergral <= -1) 
		s_PidItemYaw.intergral = -1;
...
*/
			...
			if (temp_pos_drone[2]>0.2)  //spx 初始气压计高度太大了也不好
				{
				   ...
				   cout << "Height error too high" <<endl;
				   mode_cmd_.request.custom_mode = "AUTO.LAND";
				   set_mode_client_.call(mode_cmd_);
				   LandingState = WAITING;	
				}
		...
		case PREPARE:											//起飞到指定高度
			...
			// OffboardControl_.send_pos_setpoint(posxyz_target, 0); //给偏航角的话会边转边移动
			if(command.data == 1)
			{
				//if((px4_pose_[2]<=search_alt_+0.2) && (px4_pose_[2]>=search_alt_-0.2) && detect_big == true) 
				if(detect_big == true) //please confirm the height first
				{
					//LandingState = SEARCH;
				...
				{
					//cout << "not reach height or not detect ARtag" <<endl;	
					cout << "not detect big ARtag" <<endl;	
				}
			}				
		...
		case APPROACH:
			{
				...
				if(detect_big == true)
				{
					...
					desire_xyVel_[2] = 0;
					//desire_xyVel_[2] = desire_vel_[2]; //hold height
					//desire_yawVel_ = desire_vel_[3];
					desire_yawVel_ = 0;
					cout << "APPROACH" <<endl;

					if((fabs(ar_pose_big_[0]) < 0.10) && (fabs(ar_pose_big_[1]) < 0.10)) //水平误差
					//if((fabs(ar_pose_small_[0]) < 0.10) && (fabs(ar_pose_small_[1]) < 0.10)) //水平误差
					{
						approach_count ++;
		...
		case LANDOVER:
			{
				...
				if(detect_small == true)
				{
					...
					desire_xyVel_[1] = desire_vel_[1];  
					//desire_xyVel_[2] = desire_vel_[2]; 
					desire_xyVel_[2] = landing_vel;  //这里采用pid给出的z轴速度，但其实z轴距离估计很不准，这里不如用恒速
					desire_yawVel_ = desire_vel_[3];
					//desire_yawVel_ = 0;
				...
				{	
					...
					cout << "ARUCO SMALL:small missing" <<endl;
				}

				OffboardControl_.send_body_velxyz_setpoint(desire_xyVel_,desire_yawVel_);

//				if(px4_pose_[2] <= 0.23 && (fabs(ar_pose_small_[0]) < 0.05) && (fabs(ar_pose_small_[1]) < 0.05))  //识别到的小二维码距离不是真实距离
//				if(px4_pose_[2] <= 0.23) 
				if(px4_pose_[2] <= 0.20||ar_pose_small_[2] <= 0.10) //when landing lower than 0.5m ,distance sensor is not accurate at all.camera not calibrated well and ar_pose scale 3rd
				{//ar_pose_small_[2] <= 0.3
					land_count ++;
  ...
  tf::Quaternion quat;
	for(auto &item : msg->markers)
	{
		if(item.id == markers_id_)
		{
			detect_state = true;
			detect_small = true;
      		//ar_pose_small_[0] = -(item.pose.pose.position.x - 0.0); //kinetic
      		//ar_pose_small_[1] = (item.pose.pose.position.y - 0.0);  //kinetic
...
//			cout << "markers_yaw_: "  << markers_yaw_ << endl;
		...
		if(item.id == 4)
		{
			detect_state = true;
			detect_big = true;
      		//ar_pose_big_[0] = -(item.pose.pose.position.x - 0.0) *big_marker_size/small_marker_size; //kinetic
      		//ar_pose_big_[1] = (item.pose.pose.position.y - 0.0) *big_marker_size/small_marker_size; //kinetic
...
void PX4Landing::Initialize()
{
  ...
  nh_private_.param<float>("PidZ_p", s_PidZ.p, 0.8);
  ...
  nh_private_.param<float>("big_marker_size", big_marker_size, 17.5);
  nh_private_.param<float>("small_marker_size", small_marker_size, 5.3);
  ...
  nh_private_.param<int>("Thres_count_land",Thres_count_land, 0);
  nh_private_.param<float>("landing_vel", landing_vel, -0.2);
