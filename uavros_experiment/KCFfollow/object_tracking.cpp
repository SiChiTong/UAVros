...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        }else 
        {
            ...
            for (int i=0; i<3; i++)
            {
                Command_Now.Reference_State.position_ref[i] = pos_des_prev[i] + vel_command[i]* 0.05; //节点运行频率20HZ
            }
            Command_Now.Reference_State.yaw_ref             = start_yaw/180*3.1415926;
            
            for (int i=0; i<3; i++)
            {
               // pos_des_prev[i] = Command_Now.Reference_State.position_ref[i]; //这样积分周期不准造成的误差会越来越大-spx
		pos_des_prev[i] = drone_pos[i]; //spx
            }
        }
